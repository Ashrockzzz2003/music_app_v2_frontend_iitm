...
import { SONG_URL_PREFIX, ALL_SONGS_URL, ALL_GENRES, ALL_LANGUAGES, } from '@/api';
</script>
...
<template>
    <div class="mainScreen">
        ...
        <form class="filter_box" @submit.prevent="getCreatorSongs">
            <input type="text" v-model="q" placeholder="Search Song by name, artist, genre, language, album">
            <div style="display: flex;">
                <select style="margin-right: 4px;" class="select" v-model="g">
                    <option value="-1">Select Genre</option>
                    <option v-for="genre in genreList" :key="genre.genreId" :value="genre.genreId">{{ genre.genreName }}
                    </option>
                </select>
                <select class="select" v-model="l">
                    <option value="-1">Select Language</option>
                    <option v-for="language in languageList" :key="language.languageId" :value="language.languageId">{{
    ...
    methods: {
        ...
        getCreatorSongs() {
            ...
            if (this.q.length > 0) {
                ...
                queryString += `q=${encodeURIComponent(this.q)}`;
            }
